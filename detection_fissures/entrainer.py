...
Usage :
    python -m detection_fissures.entrainer
    python -m detection_fissures.entrainer --epoques 100 --lot 8 --lr 5e-5
    python -m detection_fissures.entrainer --donnees /chemin/vers/dataset
...
Environnements supportés :
    Local  : python -m detection_fissures.entrainer
    Colab  : !python -m detection_fissures.entrainer
    Kaggle : python -m detection_fissures.entrainer
"""
...
def analyser_arguments() -> argparse.Namespace:
    ...
    analyseur.add_argument(
        ...
        help="Décroissance des poids AdamW",
    )
    analyseur.add_argument(
        "--patience",
        type=int,
    ...
    analyseur.add_argument(
        "--aire-min-masque",
        type=int,
        default=configuration.modele.aire_min_masque,
        help="Surface minimale d'un masque conservé après redimensionnement",
    )
    analyseur.add_argument(
        "--exclure-images-sans-fissures",
        action="store_true",
        help="Ignore les images sans fissure annotée (déconseillé sauf dataset spécial)",
    )
    analyseur.add_argument(
        "--seuil-score",
        type=float,
    ...
    analyseur.add_argument(
        "--degelage-backbone",
        type=int,
        default=configuration.entrainement.epoque_degelage_backbone,
        help="Époque de dégelage de layer3/layer4",
    )
    analyseur.add_argument(
        "--degelage-complet",
        type=int,
        default=configuration.entrainement.epoque_degelage_complet,
        help="Époque de dégelage complet du backbone",
    )
    analyseur.add_argument(
        "--workers",
        type=int,
        default=configuration.entrainement.nombre_workers,
...
def verifier_dataset(chemin_donnees: Path) -> None:
    ...
    for split in SPLITS_DATASET:
        ...
        try:
            ...
            erreurs.append(
                f"- JSON COCO invalide : {chemin_annotations} ({exc})"
            )
            continue
...
def main() -> None:
    ...
    config.entrainement.decroissance_poids = args.weight_decay
    config.entrainement.patience_arret_precoce = args.patience
    config.entrainement.precision_mixte = not args.sans_mixte
    config.entrainement.epoque_degelage_backbone = args.degelage_backbone
    config.entrainement.epoque_degelage_complet = args.degelage_complet
    config.entrainement.nombre_workers = args.workers
    config.modele.taille_image_min = args.taille_image
    config.modele.taille_image_max = args.taille_image
    config.modele.aire_min_masque = args.aire_min_masque
    config.modele.inclure_images_sans_fissures = not args.exclure_images_sans_fissures
    ...
    chargeurs = creer_chargeurs_donnees(
        ...
        aire_min_masque=config.modele.aire_min_masque,
        inclure_images_sans_fissures=config.modele.inclure_images_sans_fissures,
        nombre_workers=config.entrainement.nombre_workers,
        epingler_memoire=config.entrainement.epingler_memoire
        and dispositif.type == "cuda",
    )
    ...
    modele = construire_modele_masque_rcnn(
        ...
        tailles_ancres_rpn=config.modele.tailles_ancres_rpn,
        ratios_ancres_rpn=config.modele.ratios_ancres_rpn,
    )
        ...
        json.dump(
            {
                "hyperparametres": {
                    ...
                    "aire_min_masque": args.aire_min_masque,
                    "inclure_images_sans_fissures": not args.exclure_images_sans_fissures,
                    "seuil_score": args.seuil_score,
                    "architecture": args.architecture,
                    "degelage_backbone": args.degelage_backbone,
                    "degelage_complet": args.degelage_complet,
                    "graine": args.graine,
                },
